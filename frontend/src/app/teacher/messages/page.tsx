...
export default function TeacherMessagesPage() {
  ...
  useEffect(() => {
    if (selectedContact && messages.length > 0) {
      // Find if there are any unread messages from them
      const hasUnread = messages.some(m => m.sender_id === selectedContact.id && !m.is_read);
      if (hasUnread) {
        api.put(`/messages/read/${selectedContact.id}`).catch(console.error);
        // Optimistically mark local as read
        setMessages(prev => prev.map(m => 
          m.sender_id === selectedContact.id ? { ...m, is_read: true } : m
        ));
      }
    }
                ...
                {filteredContacts.map((contact, index) => (
                  ...
                  >
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg flex-shrink-0 transition-transform duration-300 group-hover:scale-105 overflow-hidden
                      ...
                      {contact.avatar_url ? (
                        <img src={getMediaUrl(contact.avatar_url)} alt={contact.username} className="h-full w-full object-cover" />
                      ) : (
                ...
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-gradient-to-br from-indigo-500 to-violet-500 text-white rounded-full flex items-center justify-center font-bold text-lg shadow-sm overflow-hidden">
                    {selectedContact.avatar_url ? (
                      <img src={getMediaUrl(selectedContact.avatar_url)} alt={selectedContact.username} className="h-full w-full object-cover" />
                    ) : (
                                ...
                                >
                                  {selectedContact?.avatar_url ? (
                                    ...
                                    <div className="w-3.5 h-3.5 rounded-full bg-gray-200 flex items-center justify-center ring-1 ring-white/50" title="Seen">
                                      <span className="text-[6px] font-bold text-gray-500">{selectedContact?.username?.charAt(0).toUpperCase()}</span>
                                    </div>
                                  )}
                                </motion.div>
                              )}
          ...
          <DialogModal
            ...
            message={
              deleteType === 'message' 
                ...
                : `Are you sure you want to permanently delete the entire conversation with ${selectedContact?.username}? This action cannot be undone.`
            }
            type="danger"
            confirmText="Delete"
