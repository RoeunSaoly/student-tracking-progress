...
import * as userRepo from "../admin/users/user.repository.js";
...
export const getStudentProfile = async (studentId, teacherId) => {
    ...
    const details = await userRepo.getStudentAcademicRecord(studentId);
    const user = await userRepo.findUserById(studentId);
